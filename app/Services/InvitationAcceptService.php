...
final readonly class InvitationAcceptService
{
    ...
    /**
     * @param  bool  $skipPasswordCheck  Skip password verification — used when the caller is
     *                                   already authenticated with the invited email (same-email auto-accept).
     */
    public function accept(TenantInvitation $invitation, AcceptInvitationData $data, bool $skipPasswordCheck = false): User
    {
        return $this->db->transaction(function () use ($invitation, $data, $skipPasswordCheck): User {
            abort_unless($invitation->isAcceptable(), 410, __('app.invitation_expired'));
            ...
            /** @var Role $role */
            $role = Role::inTenant($invitation->tenant_id)->where('name', $invitation->role_name)->firstOrFail();
