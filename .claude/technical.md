...
**Last delta:** 2026-09-08 (RecurringInvoice template period refactor: removed period_from/period_to columns from template model (template stores start_date only), removed from RecurringInvoiceUpsertData + DetailData DTOs, service now sets period_from/period_to = null on every generated invoice instead of copying stale template dates → periodic invoices carry no billing period, user fills period on generated draft if needed; FE: RecurringInvoiceForm no longer renders period fields, RecurringScheduleCard no longer shows period row; new test regression test_generated_invoice_has_null_period_fields, tests 987 total). Previous: 2026-09-07 Phase 9 Dashboard (multi-tenant overview + alerts + capped display, 950+ tests).
