...
- **Satellites:**
  ...
  - `App\Data\Invoices\{InvoiceUpsertData, InvoiceIssueData, InvoiceItemData, InvoiceListItemData, InvoiceDetailData, InvoiceSupplierData, InvoiceSettingsData, InvoiceIndexFilterData}` — DTOs per spec D2–D4. Single UpsertData for store+update (shapes identical). Single tenant-scoped `client_id`/`cleaning_object_id` exists check + "object must belong to client" closure rule.
  ...
  - `App\Http\Controllers\{InvoiceController, InvoiceSettingsController}` — thin, `#[Authorize]` per action. Index/Create/Show/Edit/Destroy + invokable-style issue/pay/cancel/duplicate (POST, redirect with flash). Settings `show`/`update` → `InvoiceSettingsService` transaction.
...
- Last delta: 2026-06-13 (PDF engine: dompdf → `spatie/laravel-pdf` `chrome` driver (`chrome-php/chrome`); real arm64 Chromium via Playwright in `docker/8.5/Dockerfile` (`/usr/local/bin/chromium-real`); `RendersInvoicePdf` DIP interface; image rebuilt + real invoice render verified (33 KB `%PDF`). `FORWARD_DB_PORT=5433` in `.env` to dodge another project on host 5432.)
