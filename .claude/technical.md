...
**Last delta:** 2026-09-06 (Tenancy UX fix: "Add another company" modal expanded with full supplier profile capture (name/ico/dic/vat_number/is_vat_payer/address_line/city/postal_code/country/contact_email/contact_phone/iban/swift_bic in 4 sections: Základné údaje/Adresa/Kontakt/Bankové údaje) + copy_settings logic (copies invoice defaults: invoice_template, recurring_default_state, default_constant_symbol/payment_type/currency/rounding_mode + invoice_number_format/vat_rate from active tenant to new tenant; colour override). IBAN/SWIFT regex patterns single-sourced in TenantSupplierProfileData::IBAN_PATTERN/SWIFT_BIC_PATTERN (reused by InvoiceSettingsData). New component TenantSupplierFields.vue. AddTenantData full DTO. RegistrationService::addTenant with copyInvoiceDefaults. TenantController::store redirect logic (dashboard if complete, settings.invoicing if incomplete). Leader invitation REMOVED (invitations only via Employees/Users). 966 tests total (tenancy +11, fix +8), PHPStan [OK] (baseline 190 unchanged), Pint clean. Tests: TenantControllerTest rewritten (21 tests, was 12). Previous: Phase 8 Notifications module, 955 tests).
