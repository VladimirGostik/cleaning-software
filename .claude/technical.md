...
**Last delta:** 2026-09-07 (Phase 9 Dashboard: DashboardService::overviewFor(User) with TenantVisibility permission snapshots (setPermissionsTeamId loop + relation-cache reset/restore finally block per ADR-D3), DashboardMetricsQuery (7 const queries per-currency invoice buckets, schedule today/week/unassigned, contracts/quotes/employees/clients/objects, withoutGlobalScope+whereIn pattern for cross-tenant), DashboardAlertsQuery (8 list+count queries, 5 alert types: overdue_invoice/supplier_incomplete/unassigned_job/contract_expiring/quote_expiring, sorted rank+urgency, capped 15/type 50 total per config/dashboard.php), no cache (ADR-D1 rationale), TenantSwitchData (optional redirect_to param). FE: Pages/Dashboard (company grid 1/2/3 cols + sticky alerts aside), Components/Dashboard/{CompanyCard,CompanyMetricGroup,MetricStat,AlertsPanel,AlertRow,alertTypeStyle,dashboardNavigation}, Composables/{useTenantSwitchVisit,useCompanyMetricGroups}, 15 lang keys (parity sk/en/uk, reuse dashboard_alert_*). Tests: 13 Feature/Dashboard/DashboardOverviewTest (shape, multi-company, isolation, gating, registrar restore) + 2 Unit/Services/DashboardServiceTest (empty membership, registrar restore) + 3 tenancy switch tests (foreign/inactive/redirect). Tests ~950+ total, PHPStan [OK] (baseline 190), Pint clean. Verified: DashboardController route GET /, DashboardService methods, TenantVisibility struct, DashboardMetricsQuery per-type queries (Postgres FILTER syntax), DashboardAlertsQuery list+count queries, DTOs (#[TypeScript]), TenantSwitchData validation, FE components + composables, lang parity. Previous: Phase 8 Notifications module (966 tests), Phase 7 Employees + Schedule (955 tests).
