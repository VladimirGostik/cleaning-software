...
**Last delta:** 2026-09-06 (Phase 5 Quotes: 664 tests, PHPStan [OK], Pint clean, FE lint/typecheck/build green; browser-verified: quotes list, create form (kind, 3 recipient modes), rough quote show with Nástrel badge, item frequency + note, VAT recap, attach-client panel. BE: Quote+QuoteItem models + QuoteStatusEnum/QuoteKindEnum enums + QuoteService (paginate/create/update/send/accept/reject/attachClient/duplicate/delete/convertToInvoice); DocumentTotalsCalculator extracted (shared by Invoice+Quote, line+totals methods, VAT breakdown grouping); Pdf/QuotePdfService + RendersQuotePdf contract; ExpireQuotes command (daily cron, Expired|Expiring events ShouldDispatchAfterCommit); Policies QuotePolicy (permission-only, 422 state guards in service); ProvidesSubjectOptions trait (clientOptions/objectOptions shared), ObjectBelongsToClient rule (DataAwareRule), TemporaryMediaConstraints rule (mime/size validation). DTOs: QuoteUpsertData (#[MergeValidationRules], clientless prohibits rule, immutable kind on edit, items array, document_uuid + OwnedTemporaryMedia); QuoteItemData (frequency nullable); QuoteListItemData (has_document eager media); QuoteDetailData (customer_name fallback, vat_breakdown, document MediaFileData, invoices QuoteInvoiceLinkData[]); QuoteFormContextData (clients/objects options, defaults); MediaFileData (shared with temp uploads). Events QuoteSent/QuoteExpired/QuoteExpiring dispatched, zero phase-5 listeners. FE: Pages Quotes/{Index,Create,Edit,Show}; components QuoteStatusBadge, QuoteKindBadge, QuoteRoughBadge, QuoteDocumentUpload (FileUploadInput pattern), QuoteAttachClientPanel (clientless only), QuoteItemsEditor (reuses InvoiceItemsEditor + frequency column), QuoteSubjectPicker. i18n: quotes, quote_status_*, quote_kind_*, quote_created/sent/accepted/rejected/deleted, quote_document_*, object_requires_client (rename invoice_object_*). Config config/quotes.php (document.disk=DOCUMENTS_DISK, max_size_kb, allowed_mimes, default_validity_days, expiring_notice_days). Invoices updated: quote_id FK (nullOnDelete), quote() BelongsTo relation, HasPdfFilename trait (extracted from Invoice), InvoiceDetailData gains quote_id/quote_number for backlink. Tests: QuoteServiceTest (22), QuoteControllerTest (24), QuotePolicyTest (12), QuoteDocumentTest (12), QuoteConversionTest (6), QuoteTotalsParityTest (2), ExpireQuotesCommandTest (8), DocumentTotalsCalculatorTest (5) = 91 new tests.)
