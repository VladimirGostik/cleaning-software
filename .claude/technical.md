...
**Last delta:** 2026-09-06 (Phase 6 Contracts: 762 tests total, PHPStan [OK] (+1 baseline entry for PendingCommand pattern), Pint clean. BE: ContractTemplate/Contract/EmploymentContract models + ContractCategoryEnum/ContractStatusEnum/ContractTermTypeEnum/EmploymentContractTypeEnum/ContractableTypeEnum enums + ContractTemplateService/ContractService/PlaceholderResolverService + ContractPdfService (RendersContractPdf interface, Blade PDF with nl2br+employment+termination blocks); CheckContractExpiry command (daily, phases 1–2: expired+expiring events). Events ContractSigned/ContractExpired/ContractExpiring (ShouldDispatchAfterCommit, zero listeners phase 6). Policies ContractPolicy (5 methods: viewAny/view/create/update+Draft/sign+Draft/terminate+Active/delete+Draft/downloadPdf) + ContractTemplatePolicy. Controllers ContractController + ContractTemplateController with #[NavItem] (Contracts top-level DocumentCheckIcon order 36; templates in settings group RectangleStackIcon order 30, D1). Routes /contract-templates/*, /contracts/*, /contracts/{id}/sign, /contracts/{id}/terminate, /contracts/{id}/pdf. QuoteService::convertToContract (Accepted itemized + client + object → Draft service_agreement, optional template, items→body via PlaceholderResolverService, quote_id set, re-conversion allowed). Quote::contracts() HasMany, QuoteDetailData.contracts added, QuoteController::show contractTemplates prop (R1). DTOs: 11 contract-related (UpsertData, ListItemData, DetailData, TerminateData, EmploymentContractUpsertData/Data, MembershipOptionData, PlaceholderTokenData, PlaceholderCatalogData, ContractFormContextData), 2 quote-related (QuoteContractLinkData, QuoteConvertToContractData). Factories ContractTemplateFactory + ContractFactory (states) + EmploymentContractFactory. FE: Pages Contracts/{Index,Create,Edit,Show}, ContractTemplates/{Index,Create,Edit,Show} + components (status/category/term badges, ContractBodyEditor with TextareaInput.insertAtCursor expose, PlaceholderTokenList grouped, ContractBodyPreview, ContractSubjectFields with category→contractable UX, EmploymentContractFields, forms + action/parties/employment/links cards, terminate modal). TextareaInput.vue extended (insertAtCursor, only ref() use). AppLayout.vue ICONS += DocumentCheckIcon, RectangleStackIcon. Enums utils (CONTRACT_STATUSES/CATEGORIES/TERM_TYPES/EMPLOYMENT_TYPES/CONTRACTABLE_TYPES + key fns + CONTRACT_CATEGORY_CONTRACTABLE map). i18n: +75 keys sk/en/uk (contract_* labels + template_* + employment_* + contractable_* + token_* + pdf_* + quote_convert_contract_* for step 9). Tests: 98 new tests across ContractTemplate{Service,Policy,Controller}, Contract{Service,Policy,Controller}, PlaceholderResolverService, ContractPdf, CheckContractExpiry command, Quote{Conversion,Policy} extensions.)
