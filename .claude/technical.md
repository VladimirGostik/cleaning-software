...
**Last delta:** 2026-09-06 (Shell UX fix: persistent Inertia layout via createInertiaApp resolver in app.ts (Auth/* + Invitations/* excluded), internal nav via Link (no full-page reload), mobile drawer closes on navigate, logout moved to sidebar footer, settings group (Roles 40/AuditLogs 50/Media 60), 955 tests (+3 NavigationRegistryTest), PHPStan [OK] (no new baseline entries), Pint clean. FE: app.ts createInertiaApp({ layout }), AppLayout persistent + drawer state management + logout footer, Header breadcrumbs Link, FormActions cancel Link, DataTable edit Link, InvoiceSettingsForm width cap removed, all 100+ internal-nav anchors converted to Link across Pages/* + Components/*. BE: RoleController/AuditLogController/MediaController #[NavItem] group:settings + order 40/50/60 attributes. Tests: NavigationRegistryTest (happy/failure/edge cases for settings group ordering, user permission gates). Previous: Phase 8 Notifications module, 952 tests).
