...
**Last delta:** 2026-09-18 (Object contacts: CleaningObject gained on-site contact persons (ObjectContact model, mirroring ClientContact); ContactCollectionSynchronizer shared service extracted (used by both clients + objects, one rule two call sites); contacts field-level gated by ViewAllObjects permission (fail-closed: null on gated actors, [] or int on permitted); write-side gate: gated actors' contact payload ignored, existing rows intact; cascade: client delete soft-deletes object contacts in same transaction; deactivate/reactivate leave contacts untouched; 3 new test files (ObjectContactTest, ObjectContactVisibilityTest, ObjectContactCascadeTest) + ContactCollectionSynchronizerTest + expanded ObjectControllerTest + ObjectServiceTest, tests 1025 total, PHPStan [OK] zero new baseline entries). Previous: 2026-09-08 RecurringInvoice template period refactor (989 tests).
