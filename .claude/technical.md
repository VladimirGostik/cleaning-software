...
**Last delta:** 2026-09-06 (Supplier profile completeness fix: 564 tests, PHPStan [OK], Pint clean, FE lint/typecheck/build green, browser-verified supplier guard + settings drawer. BE: Tenant::missingSupplierFields()/hasCompleteSupplierProfile() methods; TenantSupplierProfileData DTO with full validation parity (names, email, IBAN/SWIFT regex, country size); RegistrationService::createOwner(…, ?TenantSupplierProfileData) accepts optional supplier DTO; CreateOwner command adds 11 optional flags (--address-line, --city, --postal-code, --country, --dic, --vat-number, --vat-payer, --contact-email, --contact-phone, --iban, --swift) + interactive prompts (values-less or default); InvoiceService::issue() guard before transaction: throws ValidationException key `supplier` + __('app.invoice_supplier_incomplete') if tenant->missingSupplierFields() !== [] (no sequence consumed); InvoiceFormContextData/InvoiceDetailData expose supplier_missing_fields for FE banner; GenerateRecurringInvoiceJob D2b skip-with-warning (Draft + Log::warning + advance schedule if supplier incomplete + auto_issue=true); POST /tenants now redirects to settings.invoicing (D1) with flash; demo tenant seeded with complete VAT-payer profile (fixture data). FE (slice b3b4a10): SupplierIncompleteAlert component + InvoiceSettingsForm extracted (shared by page + drawer) + InvoiceSettingsDrawer + useInvoiceSettingsDrawer composable (fetches /settings/invoicing via Inertia protocol, handles 409 asset-version mismatch, saves via PUT + partial reload); mounted on Invoices Create/Edit/Show + RecurringInvoices Create/Edit + header button on Invoices Index. i18n: invoice_supplier_incomplete, tenant_created_complete_supplier, invoice_supplier_missing_{name,address_line,city,postal_code,ico,dic,vat_number} keys added (sk/en/uk).)
