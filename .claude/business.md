...
- **Create and manage quotes (cenové ponuky)** — Sekretárka or Vedúca creates price quotes from templates or line-item basis. Two subject modes: linked to a client, or linked to a client + object (work at a specific location). Line items with per-item VAT rate + discount percentage (no snapshot of customer data — this is deliberately left to invoices/contracts, which freeze data at issue time). Per-tenant auto-numbering `CP{YYYY}{NNNN}` with manual override. Issue date + validity period (valid_until) set at creation. Status lifecycle: Draft (editable) → Sent (stamp sent_at, manual action) → Accepted / Rejected / Expired (daily cron checks valid_until past → auto-expire). Lifecycle actions: send (marks Sent + stamps sent_at + dispatches `QuoteSent` notification to team members with `view quotes`; no customer-facing email yet), accept (opens conversion), reject (terminal), duplicate (copy as Draft), convertToInvoice (delegate to InvoiceService, links via quote_id), convertToContract (delegate to ContractService, links via quote_id). Only Accepted quotes can convert; re-conversion allowed. PDF generation (chrome driver, Blade-based, single template). Feature-gated (Pro/Enterprise tiers only). Permission-gated: `view quotes`, `create quotes`, `edit quotes`, `send quotes`, `approve quotes` (accept/reject authority), `delete quotes`, `duplicate quotes`, `convert to invoice/contract` (scoped to respective InvoicePolicy/ContractPolicy create permissions).
...
- **Manage employees** — Vlastník (full CRUD) + Vedúca upratovačka (view, assign roles only) manage tenant employees (Users bound to the tenant via TenantMembership pivot). List: filters by search (name/email/first_name/last_name), role, active status; sorts by joined date. Create: new user (auto-verified, Free plan, random password, gets InvitationCreated email) or link existing user; assign role within tenant (role assignment restricted: actor can only assign roles whose full permission set is a subset of their own — prevents privilege escalation; Vlastník unrestricted). Profile: first_name, last_name, phone, position (optional contact + display info). Permissions: optional direct overrides (actor can only grant perms they themselves hold — prevents escalation). Employment contract: optional, can start a draft employment contract (DPP/DPČ/TPP/Živnosť, position, hourly_rate/monthly_salary, weekly_hours, probation_end_date) linked to the membership. Edit: profile + role + permission overrides + contract management (not touched on update for existing contracts). Deactivate: is_active=false (soft-revoke, reactivatable). Feature-gated (Starter/Pro/Enterprise tiers). Quota: MultiUser plan limit (Free=1 owner, Starter=5, Pro=20, Enterprise=unlimited) enforced on create.
...
| Vedúca upratovačka (Supervisor) | Schedule (view/create/edit/assign), Employees (view, assign), Complaints (full), Photos (full), Locations (view). No finance, no edit/delete employees. |
...
| Účtovníčka (Accountant) | Invoices (full), Contracts (view), Clients (view), VAT settings (view). No employees. |
...
- `view quotes` / `create quotes` / `edit quotes` / `send quotes` / `approve quotes` / `delete quotes` — Vlastník (all), Sekretárka (view/create/edit/send/approve/delete), Vedúca (view only), Účtovníčka (view only). Enforced by `QuotePolicy` + `#[Authorize]` on `QuoteController`. Feature-gated `quotes` plan (Pro/Enterprise).
...
- `view employees` / `create employees` / `edit employees` / `assign employees` / `delete employees` — Vlastník (all), Vedúca (view + assign only; she can re-assign roles but cannot create/edit/delete members themselves, per spec). Enforced by `TenantMembershipPolicy` + `#[Authorize]` on `EmployeeController`. Feature-gated `employees` plan (Starter/Pro/Enterprise). `assign employees` semantic: role assignment authority (no create/edit/delete permission needed).
- `view notifications` — Vlastník (all), Vedúca, Sekretárka, Účtovníčka. Enforced by `NotificationPolicy` (viewAny + record-level update) + `#[Authorize]` on `NotificationController`. Available on all plans (no feature gate). Grants access to notification centre + bell polling.
...
- `view schedule` / `create schedule` / `edit schedule` / `assign cleaners` — Vlastník (all), Vedúca (view/create/edit/assign). Enforced by `ScheduledJobPolicy` + `#[Authorize]` on `ScheduledJobController`. Feature-gated `schedule` plan (Pro/Enterprise).

**Seeded (some UI built, rest placeholder):**

- All 6 spec roles have permission bundles for: clients, objects, **quotes** (UI built), **contracts** (UI built), **invoices** (UI built, recurring invoices, settings), + placeholder bundles for future domains (schedule, employees, complaints, photos, templates, notifications, VAT settings, subscription management). Each role's permission set defined in `RoleTemplatesSeeder::ROLE_PERMISSIONS` map.

**Spec-planned (seeded as role bundles, UI not yet built):** objects, quotes, contracts, invoices, employees, complaints, photos, templates, notifications, VAT settings, subscription, permission management. **UI built:** schedule/jobs. See the per-role defaults table above.
