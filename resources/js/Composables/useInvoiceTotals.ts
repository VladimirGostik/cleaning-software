...
export function useInvoiceTotals(items: Ref<ItemRow[]>, isVatPayer: Ref<boolean>, deposit: Ref<number>) {
    ...
    const subtotal = computed<number>(() => roundTo2(lineCalcs.value.reduce((sum, l) => sum + l.base, 0)));

    const vatAmount = computed<number>(() => roundTo2(lineCalcs.value.reduce((sum, l) => sum + l.vat, 0)));
