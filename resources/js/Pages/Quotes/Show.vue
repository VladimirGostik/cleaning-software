<script setup lang="ts">
    ...
    const isDocument = computed(() => props.quote.kind === 'document');
    const canUploadDocument = computed(() => isDraft.value && can('edit quotes'));
    ...
    <div class="page-container">
        ...
        <div class="breadcrumbs text-sm mb-4">
            <ul>
                ...
                <li class="font-mono">{{ quote.number ?? t('quotes.no_number') }}</li>
            </ul>
        </div>

        <PageHeader :title="quote.number ?? t('quotes.no_number')" :subtitle="quote.customer_name">
            <template #badges>
                <QuoteStatusBadge :status="quote.status" />
                <span class="badge badge-ghost badge-sm">{{ t(`quote_kind.${quote.kind}`) }}</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
            <!-- LEFT column -->
            <div class="space-y-6">
                <!-- Document panel — missing state is the primary action, shown above everything -->
                <QuoteDocumentPanel
                    v-if="isDocument && !quote.document"
                    :quote-id="quote.id"
                    :document="quote.document"
                    :can-upload="canUploadDocument"
                />
                ...
                <div class="card bg-base-100 shadow-sm">
                    <div class="card-body gap-6">
                        ...
                        <div>
                            ...
                            <div v-if="quote.client_id === null" class="text-sm text-base-content/70 mt-0.5">
                                <p v-if="quote.customer_street">{{ quote.customer_street }}</p>
                                <p v-if="quote.customer_postal_code || quote.customer_city">
                                    {{ quote.customer_postal_code }} {{ quote.customer_city }}
                                </p>
                                <p v-if="quote.customer_email">{{ quote.customer_email }}</p>
                            </div>
                        </div>
                        ...
                        <!-- Items table (itemized only) -->
                        <div v-if="!isDocument" class="overflow-x-auto">
                        ...
                        <!-- Summary totals (itemized only) -->
                        <div v-if="!isDocument" class="flex justify-end">
                ...
                <!-- Document panel — normal slot once a file exists -->
                <QuoteDocumentPanel
                    v-if="isDocument && quote.document"
                    :quote-id="quote.id"
                    :document="quote.document"
            ...
            <div class="space-y-4">
                ...
                <div class="card bg-base-100 shadow-sm">
                    <div class="card-body gap-2">
                        ...
                        <!-- Convert to Invoice (Accepted, itemized only) -->
                        <Can permission="create invoices">
                            <button
                                v-if="isAccepted && !isDocument"
                                type="button"
                                class="btn btn-primary btn-sm w-full justify-start"
                                :disabled="ui.processing"
                        ...
                        <!-- Convert to Contract (Accepted, itemized only, requires object) -->
                        ...
                        <!-- Download PDF (always, unless document mode with no file yet) -->
                        <a
                            v-if="!isDocument || quote.document"
                ...
                <div class="card bg-base-100 shadow-sm">
                    <div class="card-body gap-2">
                        ...
                        <div v-if="quote.client_id" class="flex items-center gap-2 text-sm">
                        ...
                        <div v-else class="flex items-center gap-2 text-sm">
                            <UserIcon class="w-4 h-4 text-base-content/40 shrink-0" />
                            <span class="text-base-content/70">{{ quote.customer_name }}</span>
                        </div>
