...
<template>
    ...
    <SupplierIncompleteAlert :missing-fields="invoice.supplier_missing_fields" @open-settings="settingsDrawer.open" />

    <div v-if="invoice.credited_invoice_id" class="alert alert-warning mb-4">
        <span>{{ t('invoice_credit_note_for') }}</span>
        <Link :href="`/invoices/${invoice.credited_invoice_id}`" class="link link-hover font-medium">
    ...
    <div v-if="invoice.credit_note_id" class="alert alert-info mb-4">
        ...
        <Link :href="`/invoices/${invoice.credit_note_id}`" class="link link-hover font-medium">
