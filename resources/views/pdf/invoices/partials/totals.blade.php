        ...
        <td style="width:40%;">
            <table style="width:100%; border-collapse:collapse;">
                ...
                <tr style="background:#f9f9f9;">
                    <td style="padding:3px 8px; font-size:11px; color:#666;">{{ __('app.invoices.vat_recap.title') }}</td>
                    <td style="padding:3px 8px; font-size:11px; color:#666; text-align:center;">{{ __('app.invoices.vat_recap.rate') }}</td>
                    <td style="padding:3px 8px; font-size:11px; color:#666; text-align:right;">{{ __('app.invoices.vat_recap.base') }}</td>
                    <td style="padding:3px 8px; font-size:11px; color:#666; text-align:right;">{{ __('app.invoices.vat_recap.vat') }}</td>
                    <td style="padding:3px 8px; font-size:11px; color:#666; text-align:right;">{{ __('app.invoices.vat_recap.total') }}</td>
                </tr>
                ...
                <tr>
                    <td colspan="{{ $invoice->is_vat_payer ? 4 : 1 }}" style="padding:4px 8px;">{{ __('app.invoices.pdf.subtotal') }}</td>
                    <td style="text-align:right; padding:4px 8px;">{{ number_format((float) $invoice->subtotal, 2, ',', ' ') }} {{ $currency }}</td>
                </tr>
                @if($invoice->is_vat_payer)
                <tr>
                    <td colspan="4" style="padding:4px 8px;">{{ __('app.invoices.pdf.vat_recap') }}</td>
                ...
                <tr>
                    <td colspan="{{ $invoice->is_vat_payer ? 4 : 1 }}" style="padding:4px 8px; color:#666; font-size:11px;">{{ __('app.invoices.pdf.rounding') }}</td>
                ...
                <tr style="font-weight:bold; font-size:14px; border-top:2px solid #333;">
                    <td colspan="{{ $invoice->is_vat_payer ? 4 : 1 }}" style="padding:6px 8px;">{{ __('app.invoices.pdf.total') }}</td>
                    <td style="text-align:right; padding:6px 8px;">{{ number_format((float) $invoice->total, 2, ',', ' ') }} {{ $currency }}</td>
                </tr>
                @if((float) $invoice->deposit > 0)
                <tr>
                    <td colspan="{{ $invoice->is_vat_payer ? 4 : 1 }}" style="padding:4px 8px;">{{ __('app.invoices.pdf.deposit') }}</td>
                ...
                <tr style="font-weight:bold; border-top:1px solid #aaa;">
                    <td colspan="{{ $invoice->is_vat_payer ? 4 : 1 }}" style="padding:6px 8px;">{{ __('app.invoices.pdf.balance_due') }}</td>
