...
<table style="width:100%; margin-top:20px; border-top:1px solid #ddd; padding-top:12px;">
    <tr>
        <td style="vertical-align:top; width:65%;">
            ...
            <strong>{{ __('app.invoice_pdf_iban') }}:</strong> {{ $invoice->supplier_iban }}<br>
            @endif
            @if($invoice->supplier_swift)
            <strong>{{ __('app.invoice_pdf_swift') }}:</strong> {{ $invoice->supplier_swift }}<br>
            @endif
            @if($invoice->variable_symbol)
            <strong>{{ __('app.invoice_pdf_variable_symbol') }}:</strong> {{ $invoice->variable_symbol }}<br>
            @endif
            @if($invoice->constant_symbol)
            <strong>{{ __('app.invoice_pdf_constant_symbol') }}:</strong> {{ $invoice->constant_symbol }}<br>
            @endif
            @if($invoice->specific_symbol)
            <strong>{{ __('app.invoice_pdf_specific_symbol') }}:</strong> {{ $invoice->specific_symbol }}<br>
            @endif
            @if($invoice->supplier_contact_email || $invoice->supplier_contact_phone)
            <br>
            @if($invoice->supplier_contact_email)
            {{ __('app.invoice_pdf_email') }}: {{ $invoice->supplier_contact_email }}<br>
            @endif
            @if($invoice->supplier_contact_phone)
            {{ __('app.invoice_pdf_phone') }}: {{ $invoice->supplier_contact_phone }}<br>
            @endif
            @endif
