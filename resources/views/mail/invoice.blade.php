...
<head>
    ...
    <title>{{ __('app.invoice_mail_subject', ['number' => $invoice->number ?? __('app.invoice_pdf_draft')]) }}</title>
    <style>
...
<div class="wrapper">
    <div class="header">
        <h1>{{ __('app.invoice_mail_heading') }}</h1>
    </div>
    <div class="body">
        <p>{{ __('app.invoice_mail_greeting', ['name' => $invoice->customer_name]) }}</p>
        <p>{{ __('app.invoice_mail_body') }}</p>
        ...
        <table style="width:100%; margin:16px 0; border-collapse:collapse;">
            @if($invoice->number)
            <tr>
                <td style="color:#888; padding:4px 0; width:140px;">{{ __('app.invoice_pdf_number') }}:</td>
                <td style="padding:4px 0;"><strong>{{ $invoice->number }}</strong></td>
            </tr>
            @endif
            @if($invoice->due_date)
            <tr>
                <td style="color:#888; padding:4px 0;">{{ __('app.invoice_pdf_due_date') }}:</td>
                <td style="padding:4px 0;">{{ $invoice->due_date->format('d.m.Y') }}</td>
            </tr>
            @endif
            @if($invoice->supplier_iban)
            <tr>
                <td style="color:#888; padding:4px 0;">{{ __('app.invoice_pdf_iban') }}:</td>
                <td style="padding:4px 0;">{{ $invoice->supplier_iban }}</td>
            </tr>
            @endif
            @if($invoice->variable_symbol)
            <tr>
                <td style="color:#888; padding:4px 0;">{{ __('app.invoice_pdf_variable_symbol') }}:</td>
                <td style="padding:4px 0;">{{ $invoice->variable_symbol }}</td>
            </tr>
            @endif
        </table>

        <p style="font-size:12px; color:#888;">{{ __('app.invoice_mail_attachment_note') }}</p>
    </div>
    <div class="footer">
