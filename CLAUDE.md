...
- **Last verified:** 2026-09-20 (Phase 10 invoicing complete; PHPStan clean on app/)
...
**Workflows:**
- **Add another company** — "Pridať novú firmu" modal (max-w-2xl, 4 sections): name, IČO (required); dic, vat_number, is_vat_payer, address_line, city, postal_code, country (optional, country default SK); contact_email, contact_phone, iban, swift_bic (optional); colour (DaisyUI presets, optional), copy_settings toggle (copies invoice defaults from current tenant: invoice_template, recurring_default_state, default_constant_symbol, default_payment_type, default_currency, default_rounding_mode, invoice_number_format, vat_rate) → RegistrationService::addTenant → new Tenant with supplier profile + session switch → if supplier profile complete: dashboard with `tenant_created` flash; else: Settings→Invoicing with `tenant_created_complete_supplier` flash (D1: onboard owner). No leader invitation created (invitations only via Employees/Users modules).
- **Invite team member** — email on Users/Create → RegistrationService::createInvitation → InvitationCreated mail (7d token) → GET /invitations/{token} → form (password if new account; password if existing account) → POST accepts → logged in, active tenant = inviting tenant.
...
- tenancy (FE) — Components/Tenants/{TenantSwitcher,AddTenantModal,TenantColorDot,TenantSupplierFields}, Forms/ColorSwatchPicker, Composables/{usePageProps,useAuthorization,useTenantTheme}, Can component, Pages/Invitations/Accept + Components/Invitations/{InvitationAcceptForm,InvitationBlockedNotice}.
- tenancy (BE) — Tenant/TenantMembership/TenantInterface/TenantInvitation models (UUIDv7 PKs, BelongsToTenant scope, **Tenant is HasMedia owner with signature collection on private disk**). TenantContextMiddleware (D5 resolution: X-Tenant-Id header → session → first active membership). RequireActiveTenant middleware (D4 guards). RegistrationService (createOwner, addTenant with full supplier profile + invoice defaults copy, bootstrapTenant), InvitationAcceptService (resolve, accept). RoleTemplatesSeeder per-tenant role bundles (6 roles). Routes: POST /tenants (auth-only, AddTenantData includes full supplier fields + copy_settings), POST /tenants/{id}/switch (TenantPolicy), GET|POST /invitations/{token} (guest, throttle 5/min). Phase 10: Tenant::registerMediaCollections() for signature collection (NOT singleFile to retain superseded uploads).
...
- objects (FE) — Pages/Objects/{Index,Show}.vue, Components/Objects/{ObjectTypeBadge,ObjectStatusBadge,ObjectForm,ObjectFormDrawer,ObjectDetailCard,ObjectAccessCard}.vue, Components/Contacts/{ContactsList,ContactsListField,contact.ts} (shared with clients), drawer-based CRUD with deactivation (is_active toggle), dedicated reactivate endpoint, ObjectAccessCard (sensitive data warning), ContactsList card (contacts on site persons, gated by ViewAllObjects permission).
- objects (BE) — CleaningObject model (UUIDv7, hybrid is_active+SoftDeletes per D1 override, LogsActivity, withTrashed client relation, HasMany contacts + HasOne primaryContact relations), ObjectContact model (UUIDv7, tenant-scoped, soft-deletes, LogsActivity, mirrors ClientContact structure), ObjectService (paginate with visibleTo actor scoping D2 fail-closed + contacts visibility gate, create/update with User $actor param, deactivate/reactivate actions leaving contacts untouched), ObjectPolicy with isVisibleTo + viewContacts guards (contacts field-level gate via ViewAllObjects permission), ObjectController with #[NavItem], ObjectTypeEnum (#[TypeScript]), ObjectContactData DTO (#[TypeScript]). Routes: GET|POST|PUT /objects{,/{object}}, POST /objects/{object}/deactivate, POST /objects/{object}/reactivate. Permissions: ViewObjects/CreateObjects/EditObjects/DeleteObjects + ViewAllObjects breadth modifier (gates contact visibility). Contacts cascade: client delete soft-deletes object contacts in same transaction; object deactivate/reactivate leave contacts intact.
- quotes (FE) — Pages/Quotes/{Index,Create,Edit,Show}.vue, Components/Quotes/{QuoteStatusBadge,QuoteKindBadge,QuoteRoughBadge,QuoteKindSelector,QuoteDocumentUpload,QuoteDocumentPanel,QuoteAttachClientPanel,QuoteSubjectPicker,QuoteManualRecipientFields,QuoteForm,QuoteFormSummary,QuoteMetaGrid,QuotePartiesBlock,QuoteActionsCard,QuoteLinksCard,QuoteConvertToContractModal}.vue, reuses InvoiceItemsEditor + InvoiceTotalsPanel patterns, FileUploadInput (temporary upload contract), attach-client panel conditionally shown (clientless only), QuoteActionsCard + QuoteLinksCard updated for contract conversion (phase 6).
...
- settings-invoicing (FE) — Pages/Settings/Invoicing.vue, Components/Invoices/{InvoiceSettingsSupplierCard,InvoiceSettingsBankCard,InvoiceNumberFormatField,InvoiceTemplatePicker,InvoiceTemplateThumbnail,InvoiceSettingsDefaultsCard,**InvoiceSettingsSignatureCard,InvoiceSignaturePreview**,InvoiceSettingsForm,InvoiceSettingsDrawer}.vue. **signatureIntent.ts** (3-state union for upload/remove/keep). InvoiceSettingsForm extracted for reuse (page + drawer). Drawer accessible from Invoices Index header button + banner CTA, allows quick profile completion without navigating away from invoice creation flow. Phase 10: signature card added (upload PNG/JPG/WebP, max 1 MB, preview + replace/remove buttons, preview reuses InvoiceTemplateThumbnail iframe).
- settings-invoicing (BE) — InvoiceSettingsService (**TemporaryUploadService injected**, **$actor + $sessionId params** for signature moveToModel), TenantInterface extended (invoice_template, recurring_default_state, default_constant_symbol/payment_type/currency/rounding_mode), Tenant extended (supplier columns: dic, vat_number, address_line, city, postal_code, country, contact_email, contact_phone, swift_bic, **signature_media_id**). Routes: GET|PUT /settings/invoicing (InvoiceSettingsData includes **signature_uuid + remove_signature fields**, Inertia props include **TenantSignatureData + InvoiceSignatureConstraintsData**), **GET /settings/invoicing/signature** (streams file inline from private disk), **GET /settings/invoicing/preview/{template}** (renders footer when present). Permission: ManageBillingSettings (owner only). Phase 10: signature upload + management, config/invoicing.php signature block (disk, max_size_kb, allowed_mimes).
...
- employees (FE) — Pages/Employees/{Index,Create,Edit,Show}.vue, Components/Employees/{EmployeeForm,EmployeeProfileFields,EmployeeProfileCard,EmployeeEmploymentSection,EmployeeRoleFields,EmployeeRoleCard,EmployeeRoleModal,EmployeeMetaCard,EmployeeActionsCard}, EmploymentContractFields reuse. AppLayout nav "Zamestnanci" order 20.
...
- schedule (FE) — Pages/Schedule/{Index,Create,Edit,Show}.vue, Components/Schedule/{JobStatusBadge,JobTypeBadge,TaskFrequencyBadge,jobStatusStyle.ts,JobList,JobCalendar (FullCalendar v6.1.21 month/week),JobForm,JobAssignPanel,JobDetailCard,JobActionsCard,JobLinksCard,WorkBreakdownView}, TimeInput.vue, Objects/Show gained read-only Rozpis prác card. Index filters are inline via the shared TableFilters (no dedicated FiltersBar component); status/type/object/assignee pass through to the calendar, search + date apply to the list only. Composable useJobCalendar. AppLayout nav "Rozvrh" order 32 + ICONS CalendarDaysIcon.
...
- dashboard (FE) — Pages/Dashboard.vue (cross-company overview grid + alerts sidebar, responsive 1/2/3 cols), Components/Dashboard/{CompanyCard,CompanyMetricGroup,MetricStat,AlertsPanel,AlertRow,alertTypeStyle,dashboardNavigation}, Composables/{useTenantSwitchVisit,useCompanyMetricGroups}, utils/enums helper dashboardAlertTypeKey, AppLayout nav "Prehľad" order 10. All companies where user has active membership + per-tenant permission-gated metric groups (invoices/schedule/contracts/people; nulled fail-closed) + merged cross-tenant alerts (5 types: overdue_invoice/supplier_incomplete/unassigned_job/contract_expiring/quote_expiring, sorted rank+urgency, capped). 15 lang keys + reuse dashboard_alert_* keys from BE.
- dashboard (BE) — GET / (auth + tenant.required, no new permission) → DashboardController → DashboardService::overviewFor(User) returns DashboardData {companies[], alerts[], alert_counts[], generated_at}. Per-company CompanyOverviewData with 4 metric groups (invoices per-currency, schedule today/week/unassigned, contracts active/expiring, people employees/clients/objects), nulled per missing permission in that tenant (fail-closed via TenantVisibility permission snapshot, setPermissionsTeamId loop + relation-cache reset/restore per ADR-D3). Alerts via DashboardAlertsQuery (5 types, status-based, service-agreements-only, unassigned 7d window, quote-expiring itemized 7d, supplier-incomplete flag), sorted + capped (15 per type, 50 total per config/dashboard.php, counts pre-cap). DashboardMetricsQuery (7 queries, per-currency invoice buckets, default-currency-first), no cache (ADR-D1). TenantSwitchData on POST /tenants/{tenant}/switch (optional redirect_to relative path, regex reject absolute). DTOs: DashboardData/CompanyOverviewData/InvoiceMetricsData/InvoiceCurrencyMetricsData/ScheduleMetricsData/ContractsMetricsData/PeopleMetricsData/DashboardAlertData/DashboardAlertCountData. Enums: DashboardAlertTypeEnum. Services: DashboardService + Dashboard/{TenantVisibility,DashboardMetricsQuery,DashboardAlertsQuery}. Tests: 13 Feature/Dashboard/DashboardOverviewTest + 2 Unit/Services/DashboardServiceTest + 3 tenancy switch tests (950+ total). Phase 9 complete (full cross-tenant capability snapshot with permission-gated metric groups, no cache, per-currency invoice summary, alert dispatching + capping, clean switch navigation).
...
- shell (FE) — Layouts/AppLayout.vue (dark sidebar + BrandMark, gradient, TenantSwitcher + AddTenantModal, colour override --color-primary, BE navigation, persistent via createInertiaApp layout resolver in app.ts, mobile drawer closes on router navigate), Layouts/Header.vue (breadcrumbs + title), Components/{BrandMark, DataTable/*, Forms/*, Auth/*, Tenants/*, Can, PermissionManager, SideDrawer, EmptyState}, ConfirmDeleteModal + useDeleteConfirm (+ confirmVariant prop phase 4), Composables/useDependentValidation (live validation of cross-field rules: Precognition only checks the field that changed and the client skips a field whose own value did not change, so a form declares its rule edges — `{ issue_date: ['due_date'] }` — and the dependents are touched + requested via `only` in the same request; wired into Invoice/Quote/Contract/Client/RecurringInvoice forms), types/index.d.ts (SharedProps collapse), vue-i18n, DaisyUI app-theme OKLCH tokens. Internal navigation via Inertia `<Link>` (no full-page load). Logout button moved from user card to sidebar footer (above language switcher, `router.post('/logout')`). Pages do not wrap `<AppLayout>` (layout is app-level, only Auth/* and Invitations/* pages render without it). Full-width main area + InvoiceSettingsForm width cap removed.

**Note:** Phases 1–9 complete (2026-09-07). Object contacts feature (2026-09-18): CleaningObject gains own contact persons (on-site správca/domovník/recepcia) via ObjectContact model + ContactCollectionSynchronizer shared service; contacts field-level gated by ViewAllObjects permission (fail-closed: cleaners see objects but no contacts); contacts soft-delete on client cascade, untouched on object deactivate. Phase 8: Notifications module (in-app centre, bell, mail prefs, 6 event listeners). Phase 9: Dashboard "Prehľad" (cross-company overview via permission-gated metric snapshots, no cache, per-currency invoice summary, 5-type alert dispatching + capping, clean switch UX with optional redirect_to param). Phase 10: Invoice signature snapshots (PDF footer with issuer name + contact info, supplier_signature_media_id FK snapshot on issue/cancel, preview inline, settings page card, new lang keys). Shell UX: persistent layout via Inertia default-layout resolver; internal nav via Link (no full-page reload); user card + bell + logout moved to sidebar footer; settings group (Roles/AuditLogs/Media orders 40/50/60); mobile drawer closes on navigate. Phase 10+ deferred: mobile portal (cleaner + supervisor), customer portal, analytics, integrations. All 9 implemented domains + object contacts: tenant-scoped (BelongsToTenant; notifications + dashboard scoped per tenant), policy-gated (RBAC-full), logged (LogsActivity), soft-deleted where appropriate (no soft-delete on WorkBreakdownTask, Notification, Dashboard, only cascade). Cleaner role (Interná upratovačka) has own-only scoping via absent "all" permissions + ScheduledJob::scopeVisibleTo / CleaningObject::scopeVisibleTo (D3 override: any assigned job reachability); contacts never visible to cleaners. 1102 tests total (4036 assertions).
...
lint.asked: true
lint.notes: |
  - PHPStan at level max with phpstan-baseline.neon (67 entries / 169 errors, regenerated 2026-09-09). Run with `--memory-limit=1G` to avoid OOM.
  Baseline burn-down done for production code: nothing under `app/` or `database/` is suppressed any more — keep it that way, a new
  `app/` entry in the baseline is a review finding, not a shortcut. Remaining entries are test-only (collect() template types,
  mixed offsets on decoded JSON, a few nullable models in fixtures). `assertInertia` closures take `AssertableInertia $page`, and
  artisan commands go through `TestCase::artisanCommand()` which narrows the `PendingCommand|int` return.

## Deployment Status
- **Deployed to production:** no
- **Last verified:** 2026-09-20 (Phase 10 invoicing: supplier signature snapshots, footer redesign, preview + settings card, 1102 tests + PHPStan clean)
