...
final class ClientUpdateTest extends TestCase
{
    ...
    public function test_update_rejects_contact_id_of_other_client(): void
    {
        $tenant = Tenant::factory()->create();
        $this->actingAsTenantUser('Admin', $tenant);
        ...
        $foreignContact = ClientContact::factory()->for($otherClient)->create(['tenant_id' => $tenant->id]);

        $response = $this->patch(route('clients.update', $client), $this->updatePayload($client, [
            'contacts' => [
                ['id' => $foreignContact->id, 'name' => $foreignContact->name, 'position' => null, 'email' => null, 'phone' => null, 'is_primary' => false],
            ],
        ]));
    ...
    public function test_update_ico_unique_ignores_self(): void
    {
        $tenant = Tenant::factory()->create();
        $this->actingAsTenantUser('Admin', $tenant);

        $client = Client::factory()->create(['tenant_id' => $tenant->id, 'ico' => '77777777']);

        $response = $this->patch(route('clients.update', $client), $this->updatePayload($client, ['ico' => '77777777']));

        $response->assertRedirect(route('clients.show', $client));
        $response->assertSessionDoesntHaveErrors('ico');
    }
}
