...
final class RecurringInvoiceServiceTest extends TestCase
{
    ...
    public function test_upsert_data_rejects_three_decimal_places_but_accepts_two(): void
    {
        ...
        $payload = fn (array $itemOverrides): array => [
            'client_id' => null,
            'cleaning_object_id' => null,
            'name' => 'Decimal Precision Recurring',
            'type' => InvoiceTypeEnum::Monthly->value,
            'template' => null,
            'frequency' => RecurringFrequencyEnum::Monthly->value,
            'day_of_month' => 15,
            ...
            'items' => [
                array_merge(
                    ['description' => 'Item', 'quantity' => 1, 'unit' => null, 'unit_price' => 10, 'discount_percent' => 0, 'vat_rate' => 0],
                    $itemOverrides,
                ),
            ],
            'constant_symbol' => null,
        ...
        foreach (['unit_price', 'quantity', 'discount_percent'] as $field) {
            ...
            foreach (['18.125', 18.125] as $rejectedValue) {
                try {
                    RecurringInvoiceUpsertData::validateAndCreate($payload([$field => $rejectedValue]));
            ...
            $acceptedTwoDecimals = RecurringInvoiceUpsertData::validateAndCreate($payload([$field => 18.13]));
            $this->assertSame(18.13, $acceptedTwoDecimals->items[0]->{$field});

            $acceptedWhole = RecurringInvoiceUpsertData::validateAndCreate($payload([$field => 18]));
